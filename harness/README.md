...
| File | What |
| --- | --- |
| `CONSTITUTION.md` | The loadable rules file: ten topics, every rule with an identifier (`KS-01`, `SEC-03`), its origin and the table that wrote it. Team-specific rules at the end. |
...
directly against `harness/CONSTITUTION.md`. Rules need an identifier in the
topic's series and an origin: the incident, the review comment or the
decision that produced them. A rule without a cause does not survive the
first discussion.
